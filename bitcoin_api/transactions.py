...
from .config.opcodes import *
from .keys import HDKey, Key, deserialize_address, Address, sign, verify, Signature
from .networks import Network
from .values import Value, value_to_satoshi

_logger = logging.getLogger(__name__)

def reverse_endian_int(value):
    value = '{:x}'.format(value)    
    value = str(value)
    
    value = bytearray(value.encode('utf8'))
    value.reverse()
...
class Transaction(object):
    ...
    def __init__(self, inputs=None, outputs=None, locktime=0, version=None,
                 ...
                 status='new', coinbase=False, verified=False, witness_type='legacy', flag=None,miner_id=1):
        """
    ...
    def signature(self, sign_id=None, hash_type=SIGHASH_ALL, witness_type=None):
        ...
        if witness_type == 'legacy' or sign_id is None:
            return self.raw_coinbase(sign_id, hash_type, 'legacy')
    ...
    def raw_coinbase(self, sign_id=None, hash_type=SIGHASH_ALL, witness_type=None):
        """
        ...
        for i in self.inputs:            
            ...
            #adding coinbase transaction identifier
            blkheight_len = (3).to_bytes(1, 'little')                
            blkheigh_data = (self.block_height).to_bytes(3, 'little')
            coinbase_data = blkheight_len+ blkheigh_data
            coinbase_data_len= (7).to_bytes(1, 'little')            
            r+=coinbase_data_len            
            r+=coinbase_data
            #adding some data to coinbase 
            r += b'ee'
            r+= (self.miner_id).to_bytes(1, 'little')
    ...
    def sign_and_update_coinbase(self, index_n=None):
        """
        Update transaction ID and resign. Use if some properties of the transaction changed
        ...
        self.version = self.version_int.to_bytes(4, 'big')        
        self.txid = self.signature_hash()[::-1].hex()
        self.size = len(self.raw_coinbase())
        self.calc_weight_units()
        self.update_totals()       
    ...
    def add_input2(self, prev_txid, output_n, keys=None, signatures=None, public_hash=b'', unlocking_script=b'',
                  unlocking_script_unsigned=None, script_type=None, address='',
                  sequence=0xffffffff, compressed=True, sigs_required=None, sort=False, index_n=None,
                  value=None, double_spend=False, locktime_cltv=None, locktime_csv=None,
                  key_path='', witness_type=None, witnesses=None, encoding=None):
